namespace :db do
  ...
  task :bootstrap => [ 'db:drop', 'db:create', 'db:schema:load', 'db:bootstrap:clear_attachments', 'db:bootstrap:load']

  namespace :bootstrap do

    desc "Delete attachment_fu files."
    task :clear_attachments => :environment do
      require 'fileutils'
      FileUtils.rm_rf( File.join(RAILS_ROOT, 'public', 'uploaded_files', 'images', '*' ) )
    end
