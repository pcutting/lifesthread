...
def update_profile_cache
  ...
  @weight_current = u.measurements.find(:first, :order => 'measured_on desc', :conditions => ['is_goal = ? AND weight > ? ', false, 0])
   @weight_goal = u.measurements.find(:first, :order => 'measured_on desc', :conditions => ['is_goal = ? AND weight > ?', true, 0]) 

  unless @weight_current.nil? 
